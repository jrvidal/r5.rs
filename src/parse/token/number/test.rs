...
use super::super::super::peekable::PeekableChars;
use std::str;

fn stream(s: &str) -> PeekableChars<str::Chars> {
    PeekableChars::from_iter(s.chars())
}

fn next_token(s: &str) -> NumberToken {
    parse_number(&mut PeekableChars::from_iter(s.chars())).ok().unwrap()
}
...
fn suffix() {
    ...
    let s = "e+1";
    let t = parse_suffix(&mut stream(s)).ok().unwrap();
    assert_suffix!(t, {
        Some(NumSign::Plus),
        ExpMarker::Default,
        "1"
...
fn suffix_bad_marker_empty() {
    let t = parse_suffix(&mut stream(""));
}

#[test]
#[should_panic]
fn suffix_bad_marker_nonexistant() {
    let t = parse_suffix(&mut stream("z"));
}
...
fn parse_real_with_integer() {
    ...
    // Other radixes
    let n = "1010111";
    let t = parse_real(&mut stream(n), Some(Radix::Binary)).ok().unwrap();
    assert_int!(t, 0, n);

    // Other radixes, case-insensitive
    let n = "10A01f1";
    let t = parse_real(&mut stream(n), Some(Radix::Hexadecimal)).ok().unwrap();
...
fn parse_real_with_integer_error() {
    // Invalid digits
    let n = "123a";
    let t = parse_real(&mut stream(n), None);
    assert!(t.is_err());

    let n = "102";
    let t = parse_real(&mut stream(n), Some(Radix::Binary));
    assert!(t.is_err());
...
fn parse_real_with_decimal_error() {
    ...
    let n = "12.3";
    let t = parse_real(&mut stream(n), Some(Radix::Octal));
    assert!(t.is_err());
